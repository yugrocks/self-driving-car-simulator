...
def start_simulation(path, screen):
    ...
    while rval:
        ...
        y_pred = model.predict(img)[0][0] #predict its class
        screen.rotate_steering(degree=int(45 - y_pred))  #rotate the steering accordingly
        print(y_pred)
    vc=None
...
def main():
    ...
    if a == '1':  #start simulation on a pre existing video on disk
        ...
        screen = Screen() 
        threading.Thread(target=partial(start_simulation,0,screen)).start()
    else:
